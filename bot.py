...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("add_cart_"):
        ...
        await query.edit_message_text(
            ...
            f"Qancha to'laysiz? (so'mda yozing)\n"
            f"Qolgan qismi qarz sifatida saqlanadi.\n\n"
            f"To'liq to'lash uchun `{total}` yozing",
            parse_mode='Markdown'
        )
        ...
        await query.edit_message_text(
            f"💳 Joriy qarz: *{user['total_debt']:,} so'm*\n\nTo'lov miqdorini kiriting (so'mda):",
            parse_mode='Markdown'
        )
        ...
        for admin_id in ADMIN_IDS:
            try:
                ...
                await context.bot.send_message(
                    ...
                    f"📱 {user_info.get('phone', 'noma\\'lum')}\n"
                    f"💵 Jami: {checkout_total:,} so'm\n"
                    f"💳 To'landi: {paid:,} so'm\n"
                    f"💰 Qarz: {debt_amount:,} so'm",
                    parse_mode='Markdown'
                )
            except Exception as e:
